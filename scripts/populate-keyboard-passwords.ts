...
async function fetchKeyboardPasswords(lockId: string, page: number): Promise<ApiResponse> {
  const response = await fetch("https://pro-server.sifely.com/v3/lock/listKeyboardPwd", {
    method: "POST",
    headers: {
...
async function populateKeyboardPasswords() {
  try {
    ...
    for (const lockProfile of lockProfiles) {
      ...
      while (hasMore) {
        const response = await fetchKeyboardPasswords(lockProfile.lockId, currentPage);

        if (response.code !== 200) {
          console.error(`API error for lockId ${lockProfile.lockId}: ${response.msg}`);
          break;
        }
          ...
          try {
            await db.keyboardPassword.upsert({
              ...
              create: {
                ...
                startDate: password.startDate ? new Date(password.startDate) : null,
            ...
            console.error(`Error processing keyboard password ${password.keyboardPwdId}:`, error);
          }
        }

        console.log(`Page ${currentPage}: Processed ${list.length} keyboard passwords`);
      ...
      console.log(`Completed processing lock profile ${lockProfile.id}. Total passwords processed: ${totalProcessed}`);
    }

    console.log("\n=== Summary ===");
    console.log(`Successfully processed keyboard passwords for ${lockProfiles.length} lock profiles`);
